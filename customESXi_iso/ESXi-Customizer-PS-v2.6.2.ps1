...
# Edited:        Josh Johnson (josh.johnson@ngc.com)
...
if ( ($v50 -and ($v51 -or $v55 -or $v60 -or $v65 -or $v67 -or $v70)) -or ($v51 -and ($v55 -or $v60 -or $v65 -or $v67 -or $v70)) -or ($v55 -and ($v60 -or $v65 -or $v67 -or $v70)) -or ($v60 -and ($v65 -or $v67 -or $v70)) -or ($v65 -and $v67 -or $v70)  -or ($v67 -and $v70)) {
...
if ( ($pkgDir -ne @()) -or $update -or ($load -ne @()) -or ($remove -ne @()) ) {
    ...
    if ($pkgDir -ne @()) {
        for ($i=0; $i -lt $pkgDir.Length; $i++ ){
            write-host "`nLoading Offline bundles and VIB files from" $pkgDir[$i] ...
            $pkgDirStr = $pkgDir[$i]
            foreach ($obundle in Get-Item $pkgDirStr\*.zip) {
            ...
            foreach ($vibFile in Get-Item $pkgDirStr\*.vib) {
